...
DEFAULT_OVERLAP_SEC   = 0.0   # set > 0 for sliding-window segmentation
...
def segment_into_buckets(
    ...
    bucket_duration_sec : length of each bucket in seconds.
                          - 5.0s  → recommended for accent classification;
                                    captures prosody, rhythm, and intonation arcs.
                          - 1–2s  → acceptable if data is scarce (more samples,
                                    less context per sample).
                          - 0.5s  → MFCC-only features only; too short for prosody.
    min_bucket_ratio    : minimum fraction of a full bucket required to keep the
                          final (partial) segment. 0.5 = keep if >= 2.5s for 5s buckets.
    overlap_sec         : overlap between consecutive buckets in seconds.
                          0.0 = non-overlapping (default).
                          Use 0.5–1.0 for data augmentation or sliding-window inference.
                          Must be strictly less than bucket_duration_sec.
    pad_last            : if True, zero-pad the final partial segment up to full
                          bucket length (so all returned arrays are the same shape).
                          if False, return it at its natural length.
    ...
    for start in range(0, len(y), hop_size):
        ...
        # drop final segment if it's too short
        ...
        # zero-pad final segment that passed the ratio check
        if len(segment) < bucket_size:
            if pad_last:
                segment = np.pad(segment, (0, bucket_size - len(segment)))
            # if not pad_last, keep at natural length
    ...
    logger.info(
        "%s | %d bucket(s) × %.1fs = %.1fs kept of %.1fs total (overlap=%.1fs).",
