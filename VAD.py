...
def run_vad(
    ...
    for start in range(0, len(y_int16), frame_size):
        ...
        # .tobytes() is faster than struct.pack for array slices
...
def extract_speech_only(
    ...
    for i, (start, end) in enumerate(speech_segments):
        # clamp to original signal length (padding from VAD may exceed it)
...
def batch_extract_speech(
    file_pairs: list[tuple[np.ndarray, int, str]],
    **kwargs,
) -> dict[str, tuple[np.ndarray, list[tuple[int, int]]] | str]:
    """
    Run extract_speech_only over multiple (signal, sr, label) tuples.
    Failures are caught per-file and reported without halting the batch.

    Parameters
    ----------
    file_pairs : list of (y, sr, identifier_string) — identifier is used for logging
    **kwargs   : forwarded to extract_speech_only()

    Returns
    -------
    dict mapping identifier → (speech_array, segments) on success | error string on failure
    """
    results = {}
    for y, sr, name in file_pairs:
        try:
            speech, segments = extract_speech_only(y, sr, **kwargs)
            results[name] = (speech, segments)
        except Exception as exc:
            logger.error("%s | FAILED: %s", name, exc)
            results[name] = str(exc)

    passed = sum(1 for v in results.values() if not isinstance(v, str))
    logger.info("Batch VAD complete: %d/%d succeeded.", passed, len(results))
    return results